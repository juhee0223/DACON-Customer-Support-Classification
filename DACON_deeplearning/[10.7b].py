# -*- coding: utf-8 -*-
"""
[10.7b]_catboost_smotenc.py
- SMOTE-NC로 소수 클래스 오버샘플링
...
def main():
    t0 = time.time()
    print("--- [10.7b] CatBoost SMOTENC 시작 ---")
    ...
    # cat features (CatBoost용 문자열 변환 + SMOTENC용 인덱스)
    cat_cols = []
    cat_idx = []
    for i,c in enumerate(["gender","subscription_type","contract_length"]):
        if c in X.columns:
            X[c] = X[c].astype(str)
    ...
    # 범주형을 카테고리 코드로 변환 (SMOTENC는 수치형 필요)
    X_enc = X.copy()
    for c in cat_cols:
        X_enc[c] = X_enc[c].astype("category").cat.codes
    Xtest_enc = Xtest.copy()
    for c in cat_cols:
        Xtest_enc[c] = Xtest_enc[c].astype("category").cat.codes

    # ===== SMOTE-NC =====
    print("SMOTENC oversampling...")
    smote_nc = SMOTENC(categorical_features=cat_idx, random_state=SEED, sampling_strategy="auto", k_neighbors=5)
    X_bal, y_bal = smote_nc.fit_resample(X_enc, y)
    X_bal = pd.DataFrame(X_bal, columns=X_enc.columns)
    print("After SMOTENC:", X_bal.shape, y_bal.shape)
    ...
    # 저장
    np.save("[10.7b]_oof_probs.npy", oof)
    np.save("[10.7b]_test_probs.npy", tet)
    pd.DataFrame({"cv_macro_f1":cv_scores}).to_csv("[10.7b]_cv_scores.csv", index=False)

    # 기본 제출
    base_pred = tet.argmax(1)
    pd.DataFrame({ID_COL:test_ids, TARGET:base_pred}).to_csv("[10.7b]_submission_smotenc.csv", index=False)
    ...
    pd.DataFrame({ID_COL:test_ids, TARGET:bias_pred}).to_csv("[10.7b]_submission_smotenc_bias.csv", index=False)

    print("\nSaved: [10.7b]_submission_smotenc.csv, [10.7b]_submission_smotenc_bias.csv, [10.7b]_cv_scores.csv")
